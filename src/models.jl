...
function lorenz63_na_func(t, u, p)
    ...
    """
    du = similar(u)
    Ψ = p["A"] * (1 / 3 * sin(2 * pi * p["f"] * t) + 1 / 3 * sin(sqrt(3) * p["f"] * t) +
                  1 / 3 * sin(sqrt(17) * p["f"] * t))
    du[1] = p["σ"] * (u[2] - u[1])
...
lorenz63_na = System(lorenz63_na_func,
                     Dict("σ" => 10, "β" => 8 / 3, "ρ" => 28, "f" => 1, "A" => 3))
...
function lorenz96_func(t, u, p)
    ...
    for i in 1:N
        du[i] = (u[mod(i + 1, 1:N)] - u[mod(i - 2, 1:N)]) * u[mod(i - 1, 1:N)] - u[i] +
                p["F"]
    end
