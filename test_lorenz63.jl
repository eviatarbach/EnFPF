...
R = diagm(0=>1e-16*ones(6))#Symmetric(diagm(0.1*diag(vec_cov)))#Symmetric(diagm([0.05, 0.05, 0.05])
...
m = mean(x[1000:end, :], dims=1)[:]
v = var(x[1000:end, :], dims=1)[:]
...
R = cov(observations[200:end, :], dims=1)
...
da_info = DA.da_cycles(ensemble=ensemble, model=model, H=vd->[(vd).^2; vd], observations=observations, integrator=integrator,
...
ensemble = x0 .+ 0.25*randn(D, ens_size)
noda_info = DA.da_cycles(ensemble=ensemble, model=model, H=vd->[(vd).^2; vd], observations=observations, integrator=integrator,
                       da_method=da_method, localization=nothing, ens_size=ens_size, Δt=Δt,
                       window=window, n_cycles=n_cycles, outfreq=outfreq,
                       model_size=model_size, R=R,
